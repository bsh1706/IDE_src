...
// turn on the LED if the distance is between dist_min and dist_max
  ...
  else if (dist_min < dist_raw <= 125){
    analogWrite(PIN_LED, 190);
  }
  else if (125 < dist_raw <= 150){
    analogWrite(PIN_LED, 127);
  }
  else if (150 < dist_raw <= 175){
    analogWrite(PIN_LED, 60);
  }
  else if (175 < dist_raw <= 200){
    analogWrite(PIN_LED, 0);
  }
  else if (200 < dist_raw <= 225){
    analogWrite(PIN_LED, 60);
  }
  else if (225 < dist_raw <= 250){
    analogWrite(PIN_LED, 127);
  }
  else if (250 < dist_raw <= 275){
    analogWrite(PIN_LED, 190);
  }
  else if (275 < dist_raw <= dist_max){
    analogWrite(PIN_LED, 255);
  }
  delay(100);
